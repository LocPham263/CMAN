...
# def mask_metrics(img1, warped_img2):
...
#     raw_loss = pearson_r
#     raw_loss = tf.reshape(tf.reduce_sum(raw_loss), (1,))

# # ---------------------------------------------------------
#     sizes = np.prod(img1.shape.as_list()[1:])
#     img1 = tf.reshape(img1, [-1, sizes])
#     warped_img2 = tf.reshape(warped_img2, [-1, sizes])

#     img1 = tf.cast(img1 > 128, tf.float32)
#     warped_img2 = tf.cast(warped_img2 > 128, tf.float32)
#     dice_score = 2.0 * tf.reduce_sum(img1 * warped_img2, axis=-1) / (
#         tf.reduce_sum(img1, axis=-1) + tf.reduce_sum(warped_img2, axis=-1))

#     return raw_loss + (dice_score - dice_score), dice_score

def similarity_metric(img1, warped_img2):
    # print(img1.shape, warped_img2.shape)
...
class RecursiveCascadedNetworks(Network):
    ...
    # Change det_factor, ortho_factor and reg_factor for your desired values 
    def __init__(self, name, framework,
                 base_network, n_cascades, rep=1,
                 det_factor=0.1, ortho_factor=0.1, reg_factor=1,
        ...
        # # Option to remove the initial registration block (affine)
        # self.stems = sum([[(self.base_network("deform_stem_" + str(i),
        #                         flow_multiplier=1.0 / n_cascades), {'raw_weight': 0})] * rep
        
        # Full option with initial registration block (affine) + main registration block
    ...
    def build(self, img1, img2, seg1, seg2, pt1, pt2):
        ...
        for stem_result, (stem, params) in zip(stem_results, self.stems):
            if 'W' in stem_result:
                ...
                # stem_result['loss'] = self.similarity_loss(
                #         img1, stem_result['warped'])
        ret = {}
        ...
        if self.framework.segmentation_class_value is None:
            ...
            warped_moving = self.reconstruction([img2, flow])

            dice_score, jacc_score = mask_metrics(seg_fixed, warped_seg_moving)
            similarity_score = similarity_metric(img1, warped_moving)
            jaccs = [jacc_score]
            dices = [dice_score]
    ...
    def regularize_loss(self, flow):
        ...
        return ret
        # return tf.convert_to_tensor(1e-6,dtype=tf.float32)

    def jacobian_det(self, flow):
        mean, var = tf.nn.moments(tf.linalg.det(tf.stack([
    ...
    #     jacob = tf.less(jacob, 0)
    #     jacob = tf.cast(jacob, tf.float32)
    #     folding = tf.reshape(tf.reduce_sum(jacob)/(128*128*128), (1,))
    #     # print(folding)
    #     return folding