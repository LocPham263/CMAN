...
class FrameworkUnsupervised:
    ...
    def __init__(self, devices, image_size, segmentation_class_value, validation=False, fast_reconstruction=False):
        ...
        if aug is None:
            ...
            print(imgs, control_fields.shape)
    ...
    def validate(self, sess, generator, keys=None, summary=False, predict=True, show_tqdm=False):
        if keys is None:
        ...
        for fd in generator:
            ...
            #     print(key)
            # print('full_results: ', len(full_results))
            # print(len(results.items()))
            # count = 0
            for k, v in results.items():
                # print(count, k, type(v), v.shape)
                # count += 1
