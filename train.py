...
parser.add_argument('--val_steps', type=int, default=100000)
...
def main():
    ...
    with tf.Session(config=tf.ConfigProto()) as sess:
        saver = tf.train.Saver(tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES), max_to_keep=60, keep_checkpoint_every_n_hours=5)
        ...
        if args.finetune is not None:
            ...
            # learningRates = [1e-5, 1e-5, 1e-5, 1e-5, 1e-5 ]
            
            # learningRates = [1e-3, 1e-3, 5e-4, 5e-4 / 2, 5e-4 / 4,
            #                    1e-4 / 8, 1e-4 / 16, 1e-4 / 32, 1e-4 / 64]
            # Training
        ...
        while True:
            ...
            if args.debug or steps % 10 == 0:
                ...
                if steps < 5000 or steps % 500 == 0:
                    ...
                    # if steps >= 60000 and loss < min_loss:
                    #     print('Min loss saved!, current min loss = ', loss)
                    #     min_loss = loss
                    #     last_save_stamp = time.time()
